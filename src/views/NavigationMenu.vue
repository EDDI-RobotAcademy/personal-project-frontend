<template>
  <v-container>
    <v-row>
      <v-col>
        <div>
          <v-app-bar class="nav" app elevation="1" color="grey lighten-5">
            <div class="d-flex align-center">
              <router-link to="/">
                <v-img :src="require('../assets/icon/coffeeground.png')" width="110" ratio="1"></v-img>
              </router-link>
            </div>
            <v-spacer></v-spacer>
            <div class="search">
              <input autocomplete="off" placeholder="Search" type="text" :value="keyword" @input="searchText"
                @keyup.enter="closeCategories" @click="showCategories = !showCategories" />
              <v-row v-if="showCategories">
                <v-col cols="12">
                  <div class="category-dropdown">
                    <div @click="navigateTo">
                      <div class="category-dropdown-item">
                        <v-img class="category-dropdown-marker" :src="require('../assets/icon/no_logo.png')" width="50"
                          ratio="1"></v-img>
            ...
            <v-row justify="end" class="d-flex align-center">
              <v-col cols="auto">
                ...
                <v-menu v-if="isLogin" offset-y open-on-hover elevation="0">
                  ...
                  <v-list>
                    <v-list-item link>
                      <v-list-item-title @click="myBoardList">
                        내 게시글 관리
                      </v-list-item-title>
                    </v-list-item>
                    <v-list-item link>
                      <v-list-item-title @click="myCommentList">
                        내 댓글 관리
                      </v-list-item-title>
                    </v-list-item>
                  </v-list>
...
export default {
  ...
  methods: {
    ...
    myBoardList() {
      this.$router.push('/my-board-list-page');
    },
    myCommentList() {
      this.$router.push('/my-comment-list-page');
    }
